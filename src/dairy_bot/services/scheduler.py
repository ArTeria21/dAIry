...
from dairy_bot.config import VIENNA_TZ, Settings
...
def setup_scheduler(bot: Bot, settings: Settings) -> AsyncIOScheduler:
    ...
    async def send_reminder() -> None:
        ...
        has_entry = await note_has_content(settings.journal_path)
        if not has_entry:
    ...
    scheduler.add_job(
        send_reminder,
        trigger=CronTrigger(hour=20, minute=0, timezone=VIENNA_TZ),
        id="daily_reminder",
